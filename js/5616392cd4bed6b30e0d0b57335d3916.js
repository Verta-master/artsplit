...
$('.js_shop_compare_all_button').attr('data-count',count);$('.js_shop_compare_all_button').attr('value',$('.js_shop_compare_all_button').attr('data-title')+' ('+count+')')});if(window.diafan_ajax){diafan_ajax.success.shop_compare_goods=function(form,response){var ui=$(form).parents(".js_shop").find(".js_shop_add_compare");if(response&&response.result==='ok'){ui.toggleClass("active")}else{ui.removeClass("active")}
$("input[name=add]",form).val(ui.hasClass("active")?0:1);return!0}}
$(document).on('click','.js_shop_additional_cost input[type="checkbox"]',function(){cacl_additional_pice($(this).parents('form'))});function cacl_additional_pice(th)
